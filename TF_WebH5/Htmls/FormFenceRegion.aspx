<%@ Page Language="C#" AutoEventWireup="true" CodeFile="FormFenceRegion.aspx.cs" Inherits="Htmls_FormFenceRegion" %>
...
//            $('#txtVeh').val(cph);        }
        ...
        {   
            ...
            var sLan = "<% =Resources.Lan.Language  %>";            if(sLan == "zh")
            {
                rowVeh = { id: 'A2', text: '<% =Resources.Lan.AdministrativeAreasFence %>', iconCls: "icon-polygon" };
                $('#tt').tree('append', {
	            parent: node.target,
	                data: rowVeh
                });
        ...
        var arrAdministrativeAreasFence =new Array();
        function GetAdministrativeAreasFence()
        {
            try
            {
                arrAdministrativeAreasFence.length = 0;
                ...
                $.ajax({
                    ...
                    dataType:'json',
                    async:true, 
                    data:{username:sUserName,Pwd:sPwd,doType:'GetAdministrativeAreasFence',loginDefaultType:1},
                    success:function(data){
                            if(data.result == "true")
                            {
                                arrAdministrativeAreasFence = data.data;
                                var node = $('#tt').tree('find', 'A2');
                                $('#tt').tree('append', {
	                                parent: node.target,
	                                data: data.data
                                });
...
//	                     $("#divState").text(e.responseText);
        ...
        function ShowArea()
        {
            ...
            if(node != undefined)
            {
                if(node.id[0] == "X")
                {
                    $.each(arrAdministrativeAreasFence,function(i,value){
                        if(value.id == node.id)
                        {
                            if(value.City == "不限")
                            {
                                AddMarkFromPoliticalOnly(value.Province,value.Province);
                            }
                            else if(value.Area == "不限")
                            {
                                var sCity = value.City;
                                if(sCity == "市辖区" || sCity == "县")
                                {
                                    sCity = "";
                                }
                                AddMarkFromPoliticalOnly(value.Province + " " + sCity,value.Province);
                            }
                            else
                            {
                                var sCity = value.City;
                                if(sCity == "市辖区" || sCity == "县")
                                {
                                    sCity = value.Province
                                }
                                AddMarkFromPoliticalOnly(sCity + " " + value.Area,value.Province);
                            }
                            return false;
                        }
                    });
				    ...
				    if (r){
					    try
                        {
                            ...
                            $.ajax({
                                ...
                                dataType:'json',
                                async:true, 
                                data:{username:sUserName,Pwd:sPwd,doType:'DelAdministrativeAreasFence',loginDefaultType:1,cid:node.id.substring(1)},
                                success:function(data){
                                        if(data.result == "true")
                                        {
                                            map.clearOverlays(); 
                                            for(var i = 0; i< arrAdministrativeAreasFence.length;i++)
                                            {
                                                if(arrAdministrativeAreasFence[i].id == node.id)
                                                {
                                                    arrAdministrativeAreasFence.splice(i,1);
                                                    break;
                                                }
                                            }
                                            $('#tt').tree('remove',node.target);
            ...
            try
            {
                if(node.id[0] == "X")
                {
                    ...
                    $.ajax({
                        ...
                        dataType:'json',
                        async:true, 
                        data:{username:sUserName,Pwd:sPwd,doType:'SaveAdministrativeAreasFenceName',loginDefaultType:1,name:node.text,cid:node.id.substring(1)},
                        success:function(data){
                                if(data.result == "true")
                                {
                                    $.each(arrAdministrativeAreasFence,function(i,value){
...
//            e1.select();  
        ...
        function RadioCheck(owner)
         {
            if(document.getElementsByName('radioPoly')[0].checked)
            {
                $("#cmbProvince").removeAttr("disabled"); 
                $("#cmbCity").attr("disabled","disabled"); 
                $("#cmbArea").attr("disabled","disabled"); 
            }
            else if(document.getElementsByName('radioPoly')[1].checked)
            {
                $("#cmbProvince").removeAttr("disabled"); 
         ...
         function ShowAreas()
         {
            ...
            sAreasPoints = "";
            sAreasCondition = "";
            if(document.getElementsByName('radioPoly')[0].checked) //省
            {
                sProvince = $("#cmbProvince").val();
                sCity = sProvince;
            }
            else if(document.getElementsByName('radioPoly')[1].checked) //市
            {
                sProvince = $("#cmbProvince").val();
                sCity = $("#cmbCity").val();
                if(sCity == "市辖区" || sCity == "县")
            ...
            else if(document.getElementsByName('radioPoly')[2].checked) //区
            {
                sProvince = $("#cmbCity").val();
                sCity = $("#cmbArea").val();
                if(sProvince == "市辖区" || sProvince == "县")
                {
                    sProvince = $("#cmbProvince").val();
                }
            }
            sAreasCondition = $("#cmbProvince").val() + "," + $("#cmbCity").val() + "," + $("#cmbArea").val();
            
            AddMarkFromPolitical(sProvince + " " + sCity,sProvince);
         }
         
         function SaveAreas()
         {
            if(sAreasCondition == $("#cmbProvince").val() + "," + $("#cmbCity").val() + "," + $("#cmbArea").val())
            {
                if(sAreasCondition.length == 0)
                {
                    ShowInfo("请先显示围栏！");
                    return;
                }
                var sName = $("#txtAreaName").val();
                if(sName.length == 0)
                {
                    ShowInfo("请输入围栏名称！");
                    return;
                }
                var sProvince = $("#cmbProvince").val();
                var sCity = $("#cmbCity").val();
                var sArea = $("#cmbArea").val();
                if(document.getElementsByName('radioPoly')[0].checked)
                {
                    sCity = "不限";
                    sArea = "不限";
                ...
                var arrAllAreaPoint =  sAreasPoints.split('-');
                var sSendLatLng = "";
                for(var i = 0; i < arrAllAreaPoint.length; i++)
                {
                    var arrPoints = arrAllAreaPoint[i].split(';');
                    for(var j = 0; j < arrPoints.length; j++)
                    {
                        var sLatLng = arrPoints[j].split(',');
                        var sLng = parseFloat(sLatLng[0]);
                        var sLat = parseFloat(sLatLng[1]);
                        var baiduXY = CheckXYGpsToBaidu(sLng, sLat);
                        sLng = baiduXY[0].lng.toFixed(6);
                        sLat = baiduXY[0].lat.toFixed(6);
                        if(i == 0 && j == 0)
                        {
                            sSendLatLng = sLat + "," + sLng; 
                        }
                        else if(j == 0)
                        {
                            sSendLatLng = sSendLatLng + "-" + sLat + "," + sLng; 
                        }
                        else
                        {
                             sSendLatLng = sSendLatLng + ";" + sLat + "," + sLng; 
                ...
                $.messager.defaults.cancel = "<% =Resources.Lan.Cancel %>";
                //$.messager.defaults = { ok: "<% =Resources.Lan.Confirm %>", cancel: "<% =Resources.Lan.Cancel %>" }; 
                $.messager.confirm('<% =Resources.Lan.Prompt %>', '确定添加行政区域？', function(r){
				    if (r){
					    try
                        {
                            var sUserName = GetCookie("username");
                            ...
                            $.ajax({
                                ...
                                dataType:'json',
                                async:true, 
                                data:{username:sUserName,Pwd:sPwd,doType:'SaveAdministrativeAreasFence',loginDefaultType:1,name:sName,points:sSendLatLng,userid: sUserID,Province:sProvince,City:sCity,Area:sArea},
                                success:function(data){
                                        if(data.result == "true")
                                        {
                                            arrAdministrativeAreasFence.push(data.data);
                                            var node2 = $('#tt').tree('find', 'A2');
                                            $('#tt').tree('append', {
	                                            parent: node2.target,
	                                            data: data.data
                                            });
...
<body oncontextmenu="return false" onload="load()"  onunload="unload()"  class="easyui-layout" id="bodyLayout" style="overflow-y: hidden; scroll:"no">
    ...
    <div region="center" split="true" border="true" style="overflow: hidden; font-family: Verdana, 微软雅黑,黑体; padding:2px; width:240px">
        <div id="divAreas" class="easyui-draggable easyui-resizable" data-options="handle:'#mytitle'" style="width:200px;height:220px;border:1px solid #ccc; z-index:9000; bottom:auto; right:10px; top:40px; left:auto; position:absolute;">
		    <div id="mytitle" style="background:#E0ECFF;padding:5px;">
		        <img style="margin-top:-5px;" alt="" src="../EasyUI/themes/icons/move.png" />
		        <font color="red"  style="font-weight:bold; "><div style="margin-top:-20px; margin-left:25px;"><% =Resources.Lan.FenceRegionManage %></div></font>
		    </div>
		    <div style="padding:5px; background-color:#E6EEF8; width:95%; height:99%;">
		        省份：<select id="cmbProvince" style="width:135px;" name="cmbProvince"></select>  
		        <br /><br />
                城市：<select id="cmbCity" style="width:135px;" name="cmbCity"></select>  
                <br /><br />
                区县：<select id="cmbArea" style="width:135px;" name="cmbArea"></select>  
                <br /><br />
                &nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;<input type="radio" id="radio1" onclick="RadioCheck(this)" name="radioPoly" value="省份" />省份
                <input type="radio" id="radio2" onclick="RadioCheck(this)" name="radioPoly" value="城市" />城市
                <input type="radio" id="radio3" onclick="RadioCheck(this)" name="radioPoly" value="区县" checked="checked" />区县
                <br /><br />
                围栏名称：<input class="easyui-textbox" style="width:115px;" type="text" name="txtAreaName" id="txtAreaName" />
		        <br /><br />
		        <a href="#;" class="easyui-linkbutton" data-options="iconCls:'icon-polygon'" onclick="return ShowAreas();">显示围栏</a> 
		        &nbsp;&nbsp;
		        <a href="#;" class="easyui-linkbutton" data-options="iconCls:'icon-save'" onclick="return SaveAreas();">保存围栏</a>     
		    </div>
	    </div>
...
//        var translateCallback = function(point) {
            ...
            CenterMarker =new BMap.Marker(point);
        ...
        if (e.drawingMode == BMAP_DRAWING_POLYLINE || e.drawingMode == BMAP_DRAWING_POLYGON || e.drawingMode == BMAP_DRAWING_RECTANGLE) {
            ...
            else if(e.drawingMode == BMAP_DRAWING_POLYLINE)
            {
                if(bDragPolygonShape || bDragPolygonShapeGB)
                {
                    var pointsDraw = "";
                    for (var i = 0; i < e.overlay.getPath().length; i++) {
                        var baiduXY = CheckXYBaiduToGps(e.overlay.getPath()[i].lng, e.overlay.getPath()[i].lat);
                        var x = baiduXY[0].lng;
                        var y = baiduXY[0].lat;
                        if (pointsDraw == "") {
                            pointsDraw = x;
