<%@ Page Language="C#" AutoEventWireup="true" CodeFile="FormBaiduMap.aspx.cs" Inherits="Htmls_FormBaiduMap" %>
...
//       $('#txtVeh').val(cph);   }
    ...
    function ClearTrack()
    {
        setTimeout(ClearTrack,5000);
        if (infoWindowVeh == undefined || !infoWindowVeh.isOpen())
        {
            trackPoint = undefined;
            if(arrTrackMonitor.length > 0)
            {
                for(var iT = 0; iT < arrTrackMonitor.length ; iT++)
                {
                    map.removeOverlay(arrTrackMonitor[iT]);
                }
                arrTrackMonitor = new Array();
            }
        }
    }
...
//        AddOrMoveObject(9398, 'wu_2D', 114.0391, 22.6501666666667, 114.0491, 22.6501666666667, 0, 248, 12, 'ACC开(熄火) 油路断开 停转;定位 GPS天线正常 电源正常 ', 1, 22, 1, '0', 1, '2013-05-15 16:27:03', '1','ffffc0');
       ...
       // 定义一个控件类,即function
	    ...
	    ZoomControl.prototype.initialize = function(map){
	      ...
	      var zoomImg = document.createElement("img");     //创建一个img元素
...
//            }
          ...
          div1.appendChild(bigImg);      //为dom添加子元素img
          
          zoomImg.width="100";  //200个像素 不用加px    
          zoomImg.src="../Img/fullscreen.png";   //给img元素的src属性赋值 
          zoomImg.width = "48";
          zoomImg.height = "48";
          zoomImg.name = "left2";
          zoomImg.style.marginLeft = "5px";
          zoomImg.onclick= function(e){
            window.parent.window.FullScreenZoom();
          }
          div1.appendChild(zoomImg); 
...
//        AddOrMoveObject(11051, 'YH41080149', 106.895183333333, 33.1436166666667, 106.905781, 33.147684, 0, 1, 12, 'ACC开(启动) 油路正常;定位 GPS天线正常 电源正常 ', 1, 22, 1, '0', 0, '2013-11-26 16:03:17', '1', '00c000');
    ...
    function fullScreen(id) {
        ...
        var status = $(element).attr('data');
                if(status == 'off'){
                    ...
                    $(element).width(widths).height('height',heights);
                    $(element).attr('data','on');
                }else{
                    $(element).width(300).height('height',300);
                    $(element).attr('data','off');
    ...
    //地图增加车辆     
    var arrTrackMonitor = new Array();//实时跟踪的线
    var trackPoint = undefined;//实时跟踪最后的点
    function AddOrMoveObject(ObjectCode, VehicleNum, oLon, oLat, Lon, Lat, NowSpeed, Direct, Zoom, StatusDes, isTrack, isNeedRevice, Addr, isSetCenter, IsUpdate, sTime, ImgState, iColor, iLetter, addressCompany, iShowState) {
        ...
        var infoText = "<table style='font-size:10pt;'>" + //style='font:9pt;'
        // "<tr height='10px'><td width='15px'></td><td nowrap></td><td>" + "</td></tr>"+
//                          "<tr style=\"display:none;\"><td width='15px'></td><td width='45px' nowrap align='right'><font color='#244FAF'>ID：</font></td><td nowrap width='150px'>" + ObjectCode + "</td></tr>" +
//                          "<tr style=\"display:none\"><td width='15px'></td><td nowrap align='right'><font color='#244FAF'><% =Resources.Lan.Plate  %>：</font></td><td nowrap>" + VehicleNum + "</td></tr>" +
                          "<tr><td width='15px'></td><td nowrap align='right'><font color='#244FAF'><% =Resources.Lan.Speed  %>：</font></td><td nowrap>" + Number(NowSpeed) + "<% =Resources.Lan.KmHour %>" + 
//                           "<div style=\"float:right; padding-right:3px;\"><a href=\"#\"    onclick='getAdd(" + Lon + "," + Lat + ");' >"
//                          + "<img border=\"0\" width=\"16px\" heigth=\"16px\" src=\"../Img/track.png\" id=\"imgtrack\" name=\"imgtrack\"  alt=\"跟踪\"/></a></div>" +
                          "</td></tr>" +
                           "<tr><td width='15px'></td><td nowrap align='right'><font color='#244FAF'><% =Resources.Lan.DataTime  %>：</font></td><td nowrap>" + sTime + "</td></tr>" +
                          "<tr" + sShowState + "><td width='15px'></td><td nowrap valign='top' align='right'><font color='#244FAF'><% =Resources.Lan.UnitStatus  %>：</font></td><td width='200px' valign='top'>" + StatusDes + "</td></tr>" +
...
//        return;
        ...
        if (infoWindowVeh != undefined && infoWindowVeh.isOpen() && infoWindowVehID == ObjectCode)
        {
            ...
            var trackPointTemp = new BMap.Point(oLon, oLat);
            if(trackPoint == undefined || trackPoint.id != ObjectCode)
            {
                trackPoint = {id:ObjectCode,point:trackPointTemp};
            }
            else
            {
                var pointsTemp = new Array();
                pointsTemp.push(trackPoint.point);
                pointsTemp.push(trackPointTemp);
                trackPoint.point = trackPointTemp;
                var polyLineNowTemp = new BMap.Polyline(pointsTemp, {strokeColor:"#f33f00", strokeWeight:5});
                map.addOverlay(polyLineNowTemp);
                arrTrackMonitor.push(polyLineNowTemp);
            }
            if(!bounds.containsPoint(trackPointTemp))
            {
                map.setCenter(trackPointTemp,map.getZoom());
            }
        }
        if (IsUpdate == 0) {
    ...
    function AddObject(mapWforGPS,ObjectCode, VehicleNum, oLon, oLat, NowSpeed, Direct, Zoom, StatusDes, isTrack, Addr, isSetCenter, info, ImgState, Color, iLetter) {
        ...
        infoWindowClick = function() { infoWindowVehID = ObjectCode; infoWindowVeh = new BMap.InfoWindow(info, { enableMessage: false, width:300 }); infoWindowVeh.redraw(); this.openInfoWindow(infoWindowVeh); };
