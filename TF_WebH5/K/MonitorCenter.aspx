<%@ Page Language="C#" AutoEventWireup="true" CodeFile="MonitorCenter.aspx.cs" Inherits="K_MonitorCenter" %>
...
//                        $('#menu5006').hide();
        ...
        var sGbKey = new Array("G2","G20","G1601","G201","G202","G204","G1602","G5006","G5505","G2002","G2003");  //  
        ...
        function ThreadOnline()
        {
            $.each(jsonVehs,function(i,value)
            {
                ...
                if(bonline)
                {
                    ...
                    var iOffLineSet = 10;
                    if(value["taxino"] == "GPRS_K1型")
                    {
                        iOffLineSet = 24 * 60 * 2;
                    }
                    if((date3 / 1000 / 60) < iOffLineSet)
                    {
            ...
            if($("#tgVeh").treegrid("getChildren",row.id).length == 0)
            {
                    ...
                    $.each(jsonVehs,function(i,value){
                        if(value.GID == row.id)
                        {
                            ...
                            if(value["online"] && value["Stop"] == 1)
                            {
                                carIcon = "icon-parking";
                            }
                            else if(value["online"])
                            {
                                carIcon = "icon-online";
                            }
                            var rowVeh = { id: value.id, team: value.team, Stop: value.Stop, online: value.online, name: value.name, sim: value.sim, taxino: value.taxino, ipaddress: value.ipaddress, iconCls: carIcon, cid: value.customid, _parentId: value.GID };
                ...
                if(!bAdd)
                {
                    ...
                    $.each(jsonVehs,function(i,value){
                        if(value.GID == row.id)
                        {
                            ...
                            var rowVeh = { id: value.id, name: value.name, Stop: value.Stop, online: value.online,team: value.team, sim: value.sim, taxino: value.taxino, ipaddress: value.ipaddress, cid: value.customid,iconCls: carIcon, _parentId: value.GID };
                            attchJson.push(rowVeh);
                        }
                    });
                    if(attchJson.length > 0)
...
//                    abValue = value.substring(0,19) + "...";         
    ...
    //                $('#tableRealTime').datagrid('deleteRow',iIndex);
        ...
        function SetOnline(value,online,p)
        {
            try
            {
                ...
                if(online)
                {
                     ...
                     if(!p && thisClass != undefined && thisClass.length > 0)
                     {
                         thisClass.removeClass("icon-offine").addClass("icon-online");
                     }
                     else if(!p)
                     {
                        thisClass = $(checkBox[0].parentNode.parentNode).find("span.icon-parking");
                        if(thisClass != undefined && thisClass.length > 0)
                        {
                            thisClass.removeClass("icon-parking").addClass("icon-online");
                        }
                     }
                     if(p)
                     {
                        if(thisClass != undefined && thisClass.length > 0)
                        {
                            thisClass.removeClass("icon-offine").addClass("icon-parking");
                        }
                        else
                        {
                            thisClass = $(checkBox[0].parentNode.parentNode).find("span.icon-online");
                            if(thisClass != undefined && thisClass.length > 0)
                            {
                                thisClass.removeClass("icon-online").addClass("icon-parking");
                            }
                        }
                     } 
                }
                    ...
                    var thisClass = $(checkBox[0].parentNode.parentNode).find("span.icon-online");
                     ...
                     else
                     {
                        thisClass = $(checkBox[0].parentNode.parentNode).find("span.icon-parking");
                         if(thisClass != undefined && thisClass.length > 0)
                         {
                             thisClass.removeClass("icon-parking").addClass("icon-offine");
                         }
                     }
                }
            }
                        ...
                        $.each(data.data,function(k,item)
                        {
                            ...
                            $.each(jsonVehs,function(i,value)
                            {
                                if(value.id == vID)
                                {
                                    ...
                                    if(bGetLastposition || (date3 / 1000 / 60) < iOffLineSet)
                                    {
                                        value["online"] = true;
                                    } 
                                    else
                                    ...
                                    var iStop = value["Stop"];
                                    value["Stop"] = dataDetail.Stop;
                                    ...
                                    if(dataDetail.Unit[0] == "1")
                                    {
                                        value["unitstatus"] = GetEnterprisePositionStatus(dataDetail.Unit);
                                    }
                                    else if(dataDetail.Unit[0] == "5")
                                    {
                                        var rowUnit = GetK1Ex(dataDetail.Ex);
                                        value["unitstatus"] = rowUnit.status;
                                        sExAlarm = rowUnit.alarm;
                                    }
                                    else if(dataDetail.Unit[0] == "2")
                                    {
                                        value["unitstatus"] = GetGbBDPositionStatus(dataDetail.Unit);
                                    }
                                    else if(dataDetail.Unit[0] == "3")
                                    {
                                        value["unitstatus"] = GetGbPositionStatus(dataDetail.Unit);
                                    }
                                    else
                                    {
                                    ...
                                    {
                                        SetOnline(value,true,false);
                                    }
                                    if(value["online"])
                                    {
                                        if(iStop == 0 && value["Stop"] == 1)
                                        {
                                            SetOnline(value,true,true);
                                        }
                                        else if(iStop == 1 && value["Stop"] == 0)
                                        {
                                            SetOnline(value,true,false);
                                        }
                                    }
...
//                    ws.send(a.buffer);  
        ...
        //企标----------------------------------------------------------------------------------------------------------
        function GetEnterprisePositionStatus(pPacketData)
        {
            try
            {
                var iIsantiTurn = -1;
...
//                    sTempStatus = sTempStatus + "密码错误报警 ";
        ...
        //部标-------------------------------------------
        function GetGbBDPositionStatus(pPacketData)
        {
            try
            {
                var sTempStatus = "";
...
//                //sb.Append(((status & 0x100000) == 0 ? ("远光关 ") : "远光开 "));
        ...
        function GetGbPositionStatus(pPacketData)
        {
            try
            {
                var sTempStatus = "";
...
//	                        window.frames[iFrame[0].name].TaskVideo();
                        ...
                        addTab('<% =Resources.Lan.RealtimeVideoMonitoring %>【' + rows[i].name + "】",'../Htmls/Cmd/YJ/FormYJVideo.aspx?cid=' + rows[i].customid + '&vehid=' + rows[i].id.substring(1));
                    }
                }
           }
        }
...
//                    }
//                }
                OpenInfoCmd("../Htmls/Cmd/YJ/FormYJRoad.aspx", 570, 455, "<% =Resources.Lan.GpsNavigation %>");
            }
        }
...
<body oncontextmenu="return false" id="bodyLayout" class="easyui-layout" style="overflow-y: hidden; scroll:no" onkeydown="javascript:keyPress(event);" onkeyup="javascript:keyRelease(event);" >
    ...
    <div region="north" split="true" border="false" style="overflow: hidden; height: 30px;
        ...
        <a id="aback" href="javascript:GotoBackEnd();">资料管理后台</a>
        ...
        <span style="padding-left:10px; font-size: 16px; "><img src="../Img/blocks.gif" width="20" height="20" align="absmiddle" /> <% =Resources.Lan.K1Title2%></span>
    ...
    <%--导航菜单--%>
     <div region="east" split="true" title="<% =Resources.Lan.TerminalToolbar  %>" style="width:255px;" id="west">
	...
	<div id="vehMenuGb" class="easyui-menu" >
