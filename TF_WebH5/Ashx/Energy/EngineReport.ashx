<%@ WebHandler Language="C#" Class="EngineReport" %>

using System;
using System.Web;

public class EngineReport : IHttpHandler
{
    ...
    public void ProcessRequest (HttpContext context) {
        ...
        System.Collections.Generic.List<Models.CEngine> lstHistory = new System.Collections.Generic.List<Models.CEngine>();
        foreach (string sThisVeh in lstVehs)
        {
            string sSql = "SELECT [VehID],[Time],EngineState,EngineRPM,EngineOilConsumptionRate from Energy where (VehID=@VehID) And ([time] >= @StartDate and [time] <= @EndDate) order by [Time]";
            ...
            else
            {
                ...
                sSql = "SELECT [VehID],[Time],EngineState,EngineRPM,EngineOilConsumptionRate from Energy where VehID in (" + sLstVeh + ") And ([time] >= @StartDate and [time] <= @EndDate) order by VehID,[Time]";
                sSql = "use Bee" + DateTime.Parse(sBeginTime).ToString("yyyyMMdd") + ";   " + sSql;
            }
            System.Collections.Generic.List<Models.CSqlParameters> lstPar = new System.Collections.Generic.List<Models.CSqlParameters>();
            Models.CSqlParameters par = new Models.CSqlParameters();
            ...
            Models.CEngine temp = new Models.CEngine();
            for (int i = 0; i < drs.Count; i++)
            {
                //[],[],[],[],[],[],[],[],[],[],[],[],[],[],[],[],[]
                System.Data.DataRow dr = drs[i];
                temp = new Models.CEngine();
                temp.Time = dr["Time"].ToString();
                temp.id = Convert.ToInt32(dr["VehID"]);
                temp.State = dr["EngineState"].ToString();
                temp.RPM = Convert.ToInt32(dr["EngineRPM"]);
                temp.OilConsumptionRate = Convert.ToInt32(dr["EngineOilConsumptionRate"]);                        
                lstHistory.Add(temp);
            }         
            if (sBatch.Equals("1"))
            {
