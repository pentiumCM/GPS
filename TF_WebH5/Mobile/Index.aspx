...
<head runat="server">
    ...
    <script type="text/javascript" language="javascript">
        ...
        arrVehDetail.push({ "key": "IpAddress","value":"<% =Resources.Lan.IpAddress %>"});
...
//			        ShowInfo('到头了，前边没有啦~~');
        ...
        var sMenuAll = new Array("5006","1308","1312","1307","1302","1303");
...
//            { 
//                ShowInfo(e.touches[0].target.innerHTML);
//                e.touches[0].target.click();
//    //        ShowInfo($(e.touches[0].target));
//            return false; 
//            };
//            document.body.addEventListener('touchstart', function(event) { 
//                event.preventDefault(); 
//            }, false);
//            $("#pVehTree")[0].addEventListener('touchstart', function(event) { 
//                event.preventDefault(); 
//                if(event.touches.length > 0)
//                {
//                    event.touches[0].target.click();
//                }
//            }, false);
            DisableRightKey();
            DisableMenuBig();
            ...
            {
	            addTab('<% =Resources.Lan.BaiduMap  %>','../MHTMLS/FormBaiduMap.aspx');
	        }
	        else
	        {
	            addTab('<% =Resources.Lan.GoogleMap  %>','../MHTMLS/FormGoogleMap.aspx');
	        }
...
//                }     
        ...
        var bTreeDblClick = false;
        function treeonDblClickRow(row){   
            bTreeDblClick = true;
            setTimeout(SetTreeDblClick,3000);
            var vID = row.id;
            if(vID[0] == 'G')
            {
                return;
        ...
        {   
            ...
            {      
                ...
                {         
                    ...
                    {         
//                        abValue = value.substring(0,19) + "...";         
                        content = '<a href="#;" onclick="$.messager.alert(\'<% =Resources.Lan.Tip %>\',\'' + value + '\',\'info\')"  onclick="$.messager.alert(\'<% =Resources.Lan.Tip %>\',\'' + value + '\',\'info\')" title="' + value + '" class="easyui-tooltip">' + abValue + '</a>';      
                    }
                    else
                    {         
                        content = '<a href="#;" onclick="$.messager.alert(\'<% =Resources.Lan.Tip %>\',\'' + value + '\',\'info\')"  onclick="$.messager.alert(\'<% =Resources.Lan.Tip %>\',\'' + value + '\',\'info\')" title="' + abValue + '" class="easyui-tooltip">' + abValue + '</a>';      
                    }  
                }   
             }   
             return content;
        ...
        {   
         ...
         {
            $.mobile.go('#divMain');
            setTimeout(function(){GotoPlaceMap(lng,lat)},1000);
         }
         
         function GotoPlaceMap(lng,lat)
         {
            try
            {
                if(lng == undefined)
                                ...
                                if(Result2_7 == 0 && Cmd2_7 == 0 && bExistGuid)
                                {
                                    for(var i = 0; i < jsonVehs.length ;i++)
                                    {
                                        if(jsonVehs[i].name == data.data[0].Cph)
                                        {
                                            ...
                                            addTab('YJ<% =Resources.Lan.RightPhotograph %>【' + jsonVehs[i].name + "】",'../MHtmls/Cmd/YJ/FormYJPhoto.aspx?cid=' + jsonVehs[i].customid + '&vehid=' + jsonVehs[i].id.substring(1) + '&guid=' + Guid2_7);
                                            return false;
                                        }
                                    }
                                }
                                ...
                                if(Result2_7 == 0 && Cmd2_7 == 1)
                                {
                                    for(var i = 0; i < jsonVehs.length ;i++)
                                    {
                                        if(jsonVehs[i].name == data.data[0].Cph)
                                        {
                                            ...
                                            addTab('<% =Resources.Lan.RealtimeVideoMonitoring %>【' + jsonVehs[i].name + "】",'../MHtmls/Cmd/YJ/FormYJVideo.aspx?cid=' + jsonVehs[i].customid + '&vehid=' + jsonVehs[i].id.substring(1) + '&guid=' + Guid2_7);
                                            return false;
                                        }
                                    }
                                }
...
//                bByte = (Array(8).join(0) + bByte).slice(-8);
        ...
        function BindPolygonVehP(node)
        {
            ...
            PolygonVehs = node;
            OpenInfoCmd("../MHTMLS/Cmd/FormBindPolygonVeh.aspx", 550, 455, "<% =Resources.Lan.FenceBindVehicle %>");
        }
        ...
        function HistoryTrack(mapType)
        {
            ...
            if(rows.length > 0)
            {
                ...
                if(mapType == 1)
                {
                    addTab('<% =Resources.Lan.BaiduMap  %><% =Resources.Lan.RightTrackPlayback  %>','../MHTMLS/FormHistoryQuery.aspx?VehID=' + rows[0].id);
                }
                else if(mapType == 2)
                {
                    addTab('<% =Resources.Lan.GoogleMap  %><% =Resources.Lan.RightTrackPlayback  %>','../MHTMLS/FormHistoryGoogleQuery.aspx?VehID=' + rows[0].id);
                }
            }
        }
        ...
        function TaskVideo(owner)
        {
            ...
            if ( !$(sMenumName).menu("getItem", $(owner)).disabled ) {   
                ...
                {
                    $.mobile.back();
                    OpenInfoCmd("../MHtmls/Cmd/YJ/FormYJVideoCmd.aspx", 490, 355, "<% =Resources.Lan.RealtimeVideoMonitoring %>");
//                    for(var i =0; i < rows.length; i++)
//                    {
...
//	                    }
//                        addTab('<% =Resources.Lan.RealtimeVideoMonitoring %>【' + rows[i].name + "】",'../MHTMLS/Cmd/YJ/FormYJVideo.aspx?cid=' + rows[i].cid + '&vehid=' + rows[i].id.substring(1));
//                    }
                }
           }
        }
            ...
            if ( !$(sMenumName).menu("getItem", $(owner)).disabled ) {                
                ...
                if(rows.length > 0)
                {
                    ...
                    for(var i =0; i < rows.length; i++)
                    {
                        if(terType == 2)
                        {
                            addTab('<% =Resources.Lan.RightPhotograph %>【' + rows[i].name + "】",'../MHTMLS/Cmd/GB/FormGBPhotoList.aspx?taxino=' + rows[i].taxino + '&vehid=' + rows[i].id.substring(1));
                        }
                        else if(terType == 4)
                        {
                            OpenInfoCmd("../MHtmls/Cmd/YJ/FormYJPhotoCmd.aspx", 490, 355, "<% =Resources.Lan.RightPhotograph %>");
                            break;
...
//	                        }
//                            addTab('YJ<% =Resources.Lan.RightPhotograph %>【' + rows[i].name + "】",'../MHTMLS/Cmd/YJ/FormYJPhoto.aspx?cid=' + rows[i].cid + '&vehid=' + rows[i].id.substring(1));
                        }
                    }
                    if(terType == 2)
                    {
                        OpenInfoCmd("../MHTMLS/Cmd/GB/FormGBPhoto.aspx", 450, 355, "<% =Resources.Lan.RightPhotograph %>");
                    }
                }
            }
        }
        ...
        function AccOnSetting(owner, terType)
        {
            ...
            if ( !$(sMenumName).menu("getItem", $(owner)).disabled ) {
                ...
                if(terType == 5)
                {
                    OpenInfoCmd("../MHtmls/Cmd/K1/FormTimeComesBack.aspx", 450, 355, "<% =Resources.Lan.TimingTimeComesBack %>");
                    return;
                }
//                var sForm = '<iframe name="accFrame" scrolling="auto" frameborder="0"  src="../MHTMLS/Cmd/Enterprise/FormAccOn.aspx" style="width:100%;height:100%;"></iframe>';
                OpenInfoCmd("../MHTMLS/Cmd/Enterprise/FormAccOn.aspx", 450, 355, "<% =Resources.Lan.RightAccOnTimingInterval %>");
            }
        }        
        
        function OilSetting(owner)
        {
            OpenInfoCmd("../MHtmls/Cmd/FormOilSetting.aspx", 450, 405, "<% =Resources.Lan.OilTestingParametersSetting %>");
        }
        ...
        function SchedulingCmdSetting(owner, terType)
        {
            ...
            if ( !$(sMenumName).menu("getItem", $(owner)).disabled ) {
                ...
                if(terType == 5)
                {
                    OpenInfoCmd("../MHtmls/Cmd/K1/FormSchedulingCmdSetting.aspx", 450, 355, "<% =Resources.Lan.SendSchedulingInformation %>");
                    return;
                }
//                var sForm = '<iframe name="accFrame" scrolling="auto" frameborder="0"  src="Htmls/Cmd/Enterprise/FormAccOn.aspx" style="width:100%;height:100%;"></iframe>';
                OpenInfoCmd("../MHtmls/Cmd/Enterprise/FormSchedulingCmdSetting.aspx", 450, 355, "<% =Resources.Lan.SendSchedulingInformation %>");
            }
        }
...
//                var sForm = '<iframe name="accFrame" scrolling="auto" frameborder="0"  src="../MHTMLS/Cmd/Enterprise/FormAccOn.aspx" style="width:100%;height:100%;"></iframe>';
                if(terType == 1)
                {
                    OpenInfoCmd("../MHTMLS/Cmd/Enterprise/FormAccOff.aspx", 450, 355, "<% =Resources.Lan.RightAccOffTimingInterval %>");
                }
                else if(terType == 2)
                {
                  OpenInfoCmd("../MHTMLS/Cmd/GB/FormGBAccOff.aspx", 450, 355, "<% =Resources.Lan.RightAccOffTimingInterval %>");
                }
            }
        }
...
//                var sForm = '<iframe name="accFrame" scrolling="auto" frameborder="0"  src="Htmls/Cmd/Enterprise/FormAccOn.aspx" style="width:100%;height:100%;"></iframe>';
                if(terType == 1)
                {
                    OpenInfoCmd("../MHtmls/Cmd/Enterprise/FormOverSpeedSet.aspx", 450, 355, "<% =Resources.Lan.OverSpeedSetting %>");
                }
                else if(terType == 2 || terType == 4)
                {
                  OpenInfoCmd("../MHtmls/Cmd/GB/FormGBOverSpeedSet.aspx", 450, 355, "<% =Resources.Lan.OverSpeedSetting %>");
                }
            }
        }
        ...
        function ShutdownSetting(owner, terType)
        {
            ...
            if ( !$(sMenumName).menu("getItem", $(owner)).disabled ) {
                ...
                {
                    var sGuid =  Guid.NewGuid();
                    var sSend = "[{'key':'3_1','ver':1,'rows':0,'cols':3,data:[{'terType':" + terType + ",'name':'YJShutdown','ip':'" + rows[i].ipaddress + "','Source':2}]}]";
                    ws.send(sSend);
                    var arrInteractive = new Array();
...
//                    }
//                }
                OpenInfoCmd("../MHTMLS/Cmd/YJ/FormYJRoad.aspx", 570, 455, "<% =Resources.Lan.GpsNavigation %>");
            }
        }
        ...
        function SendCmdYJPhoto(sCmdType, iChannel, iResolution, iPhotoCount, iShootingInterval, iImageQuality, terType, sInteractive, sContent)
        {
            ...
            {
                var sGuid =  Guid.NewGuid();
                var sSend = "[{'key':'3_1','ver':1,'rows':0,'cols':3,data:[{'terType':" + terType + ",'name':'" + sCmdType + "','ip':'" + rows[i].ipaddress + "','Channel':" + iChannel + ",'Resolution':" + iResolution + ",'PhotoCount':" + iPhotoCount + ",'ShootingInterval':" + iShootingInterval + ",'ImageQuality':" + iImageQuality + ",'Source':2,'Guid':'" + sGuid.ToString("N") + "'}]}]";
                ws.send(sSend);
        ...
        function SendCmdYJRoad(sCmdType, sLng1, sLat1, sPlace1, sLng2, sLat2, sPlace2, terType, sInteractive, sContent)
        {
            ...
            for(var i = 0; i < rows.length; i++)
            {
                var sSend = "[{'key':'3_1','ver':1,'rows':0,'cols':3,data:[{'terType':" + terType + ",'name':'" + sCmdType + "','ip':'" + rows[i].ipaddress + "','Lng1':" + sLng1 + ",'Lat1':" + sLat1 + ",'Add1':'" + sPlace1 + "','Lng2':" + sLng2 + ",'Lat2':" + sLat2 + ",'Add2':'" + sPlace2 + "','Source':2}]}]";
                ws.send(sSend);
                var arrInteractive = new Array();
                var sGuid =  Guid.NewGuid();
                var arrInteractive = { id: sGuid.ToString("N"),name:rows[i].name,time:sTime, interactive:sInteractive,content: sContent + "&nbsp;&nbsp;<% =Resources.Lan.SuccessfullySentToTheCenter %>"};
        ...
        function SendCmdYJVideo(sCmdType, iChannel, iResolution, iUpload, iShootingInterval, iImageQuality, terType, sInteractive, sContent)
        {
            ...
            for(var i = 0; i < rows.length; i++)
            {
                ...
                var sSend = "[{'key':'3_1','ver':1,'rows':0,'cols':3,data:[{'terType':" + terType + ",'name':'" + sCmdType + "','ip':'" + rows[i].ipaddress + "','Channel':" + iChannel + ",'Resolution':" + iResolution + ",'Upload':" + iUpload + ",'ShootingInterval':" + iShootingInterval + ",'ImageQuality':" + iImageQuality + ",'Source':2}]}]";
                ws.send(sSend);
                var arrInteractive = new Array();
                var sGuid =  Guid.NewGuid();
                var arrInteractive = { id: sGuid.ToString("N"),name:rows[i].name,time:sTime, interactive:sInteractive,content: sContent + "&nbsp;&nbsp;<% =Resources.Lan.SuccessfullySentToTheCenter %>"};
...
//                });//                $('#openRoleDiv').window('open').window('center');              $.mobile.go('#openRoleDiv');              $('#openXXXIframe')[0].src= url;        }
    ...
    //                setTimeout(SetSelectVehToMap,2000);
        ...
        {
//            setTimeout("SetFindUnitTime('" + sTitle + "','" + sRegion + "','" + sUnit + "','" + rowSelect + "')",1000);
               setTimeout(function(){SetFindUnitTime(sTitle,sRegion,sUnit,rowSelect,bShow);},1500);
        }
        
        function SetFindUnitTime(sTitle, sRegion, sUnit, rowSelect,bShow)
        {
            try
            {
        ...
        {
             OpenInfoCmd("../MHtmls/FormNanJingUnit.aspx?t=" + title, 620, 440, "选择单位");
        }
    </script>
</head>
<body oncontextmenu="return false" ><%--style="-webkit-touch-callout: none;" --%>
    ...
    <div class="easyui-navpanel" id="divMain" style="-webkit-overflow-scrolling: touch;" >
            <%--<div id="divSeartch"  class="m-leftNoHeight"><!--m-rightbottom-->
					<a href="javascript:void(0)" class="easyui-menubutton" data-options="iconCls:'icon-onlineVeh',menu:'#mmVehTree',menuAlign:'right',hasDownArrow:false"></a>
						        </div>--%>	        <header>			<div class="m-toolbar">				<div class="m-left">				    <a href="#;" class="easyui-menubutton" data-options="iconCls:'icon-onlineVeh',menu:'#mainBase',menuAlign:'right',hasDownArrow:false"><% =Resources.Lan.BasicFunction%></a>				    <a href="#;" class="easyui-menubutton" data-options="iconCls:'icon-tip',menu:'#mainInfo',menuAlign:'right',hasDownArrow:false" ><% =Resources.Lan.Information %></a>				</div>			</div>		    </header>	        <div id="tabs" class="easyui-tabs" data-options="fit:true,border:false,tabHeight:35,onSelect:TabonSelect" ><%--,tools:'#tab-tools'--%>
		          <%--  <div title="<% =Resources.Lan.BaiduMap  %>" style="overflow:hidden;" id="home">
			            <iframe name="mainFrame" scrolling="auto" frameborder="0"  src="../MHTMLS/FormBaiduMap.aspx" style="width:100%;height:100%;"></iframe>';
            	        
		            </div>--%>
		        </div>	        <%--<div title="<% =Resources.Lan.BaiduMap  %>" style="overflow:hidden; width:100%; height:100%" id="home">
			       <iframe name="mapmainFrame" frameborder="0"  src="../MHTMLS/FormBaiduMap.aspx" style="width:100%;height:100%;">
			       </iframe>
		    </div>--%>	
            <footer style="padding:2px 3px">
                <div id="divState" style="width:100%;color: red;height:32px;font-weight: bold;"></div>
            </footer>
	</div>
    <%--导航菜单--%>
     <div id="pVehTree" class="easyui-navpanel" style="-webkit-overflow-scrolling: touch;" >
        <header>
            <div class="m-toolbar">
                <div class="m-title"><% =Resources.Lan.VehList %></div>
                <div class="m-left">
        ...
        <div id="tabNavigation" class="easyui-tabs" data-options="onSelect:VehonSelect" tabPosition="bottom" fit="true" border="false" >
		      <div id="divClick" title="<% =Resources.Lan.VehList %>" style="overflow:auto; height:100%;"><%--style="overflow:hidden; -webkit-user-select:none; -webkit-touch-callout:none;"--%>
     ...
     <%--内容--%>
     <div id="pRealTime" class="easyui-navpanel" style="-webkit-overflow-scrolling: touch;" >
        <header>
            <div class="m-toolbar">
                <div class="m-left">
                    <a href="#;" onclick="$.mobile.go('#divMain');" class="easyui-linkbutton m-back" plain="true" outline="true"><% =Resources.Lan.Back %></a>
        ...
        <table id="tableRealTime" class="easyui-datagrid" style="height:100%" data-options="idField:'id',singleSelect:true,collapsible:true,remoteSort:false,multiSort:true,onRowContextMenu: onVehGridMenu,onHeaderContextMenu: onVehGridMenu">
		    <thead>
		        <tr>
     ...
     <div id="pAlarmInformation" class="easyui-navpanel" style="-webkit-overflow-scrolling: touch;" >
        <header>
            <div class="m-toolbar">
                <div class="m-left">
                    <a href="#;" onclick="$.mobile.go('#divMain');" class="easyui-linkbutton m-back" plain="true" outline="true"><% =Resources.Lan.Back %></a>
     ...
     <div id="pInteractive" class="easyui-navpanel" style="-webkit-overflow-scrolling: touch;" >
        <header>
            <div class="m-toolbar">
                <div class="m-left">
                    <a href="#;" onclick="$.mobile.go('#divMain');" class="easyui-linkbutton m-back" plain="true" outline="true"><% =Resources.Lan.Back %></a>
        ...
        <table id="tableInteractive" class="easyui-datagrid" style="height:100%" data-options="idField:'id',singleSelect:true,collapsible:true,onRowContextMenu: onInteractiveGridMenu,onHeaderContextMenu: onInteractiveGridMenu, onDblClickRow: onVehDblClickRow">
		    <thead>
		        <tr>
		            <th data-options="field:'id',width:50,hidden:true,halign:'center'" >ID</th>
		            <th data-options="field:'name',width:100,align:'left',halign:'center' "><% =Resources.Lan.Plate %></th>
    ...
    <div id="openRoleDiv" class="easyui-navpanel"  style="-webkit-overflow-scrolling: touch;" >
        <header>
            <div class="m-toolbar">
                <div class="m-left">
                    <a href="#;" onclick="$.mobile.go('#pVehTree');" class="easyui-linkbutton m-back" plain="true" outline="true"><% =Resources.Lan.Back %></a>
                </div>
            </div>
        </header>
        <iframe scrolling="no" id='openXXXIframe' name="openXXXIframe" frameborder="0"  src="" style="width:99%;height:99%; padding:0px"></iframe>
    </div>  
    <div id="tab-tools"  class="easyui-menu">
	...
	<div id="tab-tools2"  class="easyui-menu">
		<a href="#;" class="easyui-menubutton" data-options="iconCls:'icon-help'">Help</a>
    		        
	</div>
	
	<div id="mmVehTree" class="easyui-menu" style="width:150px;">
		<div id="divSetVeh" data-options="iconCls:'icon-online'" onclick="$.mobile.go('#pVehTree');" >选择车辆</div><!--onclick="InitTree();"-->
		<%--<div id="divClearAll" data-options="iconCls:'icon-redo'" onclick="ClearAll();">清空所选车辆</div>--%>
	...
	<div id="mainBase" >
	    <div data-options="iconCls:'icon-home'">
			<span><% =Resources.Lan.BasicFunction %></span>
			<div>
			    <div onclick="$.mobile.go('#pVehTree');" data-options="iconCls:'icon-onlineVeh'"><% =Resources.Lan.VehList %></div>
			</div>
	    </div>
		<div data-options="iconCls:'icon-map'">
			...
			<div>
				<div onclick="addTab('<% =Resources.Lan.BaiduMap  %>','../MHTMLS/FormBaiduMap.aspx')" data-options="iconCls:'icon-baidu'"><% =Resources.Lan.BaiduMap %></div>
				<div onclick="addTab('<% =Resources.Lan.GoogleMap  %>','../MHTMLS/FormGoogleMap.aspx')" data-options="iconCls:'icon-googlemap'"><% =Resources.Lan.GoogleMap %></div>
				<%--<div class="menu-sep"></div>
				<div>New Toolbar...</div>--%>
			</div>
		</div>
		<div data-options="iconCls:'icon-map'" id="menu5006">
			...
			<div>
				<div onclick="addTab('<% =Resources.Lan.BaiduMap  %><% =Resources.Lan.RightTrackPlayback %>','../MHTMLS/FormHistoryQuery.aspx')" data-options="iconCls:'icon-baidu'"><% =Resources.Lan.BaiduMap %></div>
				<div onclick="addTab('<% =Resources.Lan.GoogleMap  %><% =Resources.Lan.RightTrackPlayback %>','../MHTMLS/FormHistoryGoogleQuery.aspx')" data-options="iconCls:'icon-googlemap'"><% =Resources.Lan.GoogleMap %></div>
			</div>
		</div>
		<div data-options="iconCls:'icon-report'" id="reportAll">
			<span><% =Resources.Lan.ReportFunction %></span>
			<div>
				<div id="menu1308" onclick="addTab('<% =Resources.Lan.ImageReport  %>','../MHTMLS/Reports/FormImageReportQuery.aspx')" data-options="iconCls:'icon-photo'"><% =Resources.Lan.ImageReport %></div>
				<div id="menu1312" onclick="addTab('<% =Resources.Lan.MileageReport  %>','../MHTMLS/Reports/FormMileReportQuery.aspx')" data-options="iconCls:'icon-mile'"><% =Resources.Lan.MileageReport %></div>
				<div onclick="addTab('<% =Resources.Lan.VehFenceScheduleTimeReport  %>','../MHTMLS/Reports/FormNotInPolygonQuery.aspx')" data-options="iconCls:'icon-polygon'"><% =Resources.Lan.VehFenceScheduleTimeReport %></div>
			    <div id="menu1307" onclick="addTab('<% =Resources.Lan.ReportSpeedSift  %>','../MHtmls/Reports/FormSpeedSiftReportQuery.aspx')" data-options="iconCls:'icon-speed'"><% =Resources.Lan.ReportSpeedSift %></div>
			    <div id="menu1302" onclick="addTab('<% =Resources.Lan.ReportAcc %>','../MHtmls/Reports/FormAccReportQuery.aspx')" data-options="iconCls:'icon-acc'"><% =Resources.Lan.ReportAcc %></div>
			    <div id="menu1303" onclick="addTab('<% =Resources.Lan.ReportAlarm %>','../MHtmls/Reports/FormAlarmReportQuery.aspx')" data-options="iconCls:'icon-alarm'"><% =Resources.Lan.ReportAlarm %></div>
			</div>
		</div>
		<%--<div id="divFence14" data-options="iconCls:'icon-polygon'">
			<span><% =Resources.Lan.LocationManager %></span>
			<div>
				<div id="divFence1418" onclick="addTab('<% =Resources.Lan.FenceRegionManage  %>','../MHTMLS/FormFenceRegion.aspx')" data-options="iconCls:'icon-polygon'"><% =Resources.Lan.FenceRegionManage %></div>
			</div>
		</div>--%>
	</div>
	
	<div id="mainInfo" >
	    <div onclick="$.mobile.go('#pRealTime');" ><% =Resources.Lan.RealtimeInformation %></div>
	    <div onclick="$.mobile.go('#pAlarmInformation');" ><% =Resources.Lan.AlarmInformation %></div>
	    <div onclick="$.mobile.go('#pInteractive');" ><% =Resources.Lan.InteractiveContent %></div>
	</div>
	...
	<div id="vehMenuGb" class="easyui-menu unselectable" >
		<div id="G20" class="unselectable" data-options="iconCls:'icon-control'"><span><% =Resources.Lan.RightTerminalControl%></span>
		    <div>
		        <div id="G1601" class="unselectable" onclick="CallName(this,2);" data-options="iconCls:'icon-language'"><% =Resources.Lan.RightCallName%></div>
		        <div id="G1602" class="unselectable" onclick="TaskPic(this,2)" data-options="iconCls:'icon-photo'"><% =Resources.Lan.RightPhotograph%></div>
		    </div>
		</div>
		<div id="G2" class="unselectable" data-options="iconCls:'icon-setting'"><span><% =Resources.Lan.RightTerminalSettings%></span>
		    <div>
		        <div id="G201" class="unselectable" onclick="AccOnSetting(this,2);" data-options="iconCls:'icon-time'"><% =Resources.Lan.RightAccOnTimingInterval%></div>
		        <div id="G202" class="unselectable" onclick="AccOffSetting(this,2);" data-options="iconCls:'icon-time'"><% =Resources.Lan.RightAccOffTimingInterval%></div>
		        <div id="G204" class="unselectable" onclick="OverSpeedSetting(this,2);" data-options="iconCls:'icon-speed'"><% =Resources.Lan.OverSpeedSetting %></div>
		        <div id="G2002" class="unselectable" onclick="OilOpenOrClose(this,2,'0');" data-options="iconCls:'icon-text'"><% =Resources.Lan.OilCutOff %></div>
		        <div id="G2003" class="unselectable" onclick="OilOpenOrClose(this,2,'1');" data-options="iconCls:'icon-text'"><% =Resources.Lan.OilOpen %></div>
		    </div>
		</div>
		<div id="G53" class="unselectable" data-options="iconCls:'icon-online'"><span><% =Resources.Lan.RightVehicleInformationCommand %></span>
		    <div>
		        <div id="G5006" class="unselectable" data-options="iconCls:'icon-LineRoad1'"><span><% =Resources.Lan.RightTrackPlayback %></span> 
		            <div>
		                <div class="unselectable" onclick="HistoryTrack(1);" data-options="iconCls:'icon-LineRoad1'"><% =Resources.Lan.BaiduMap %></div>
		                <div class="unselectable" onclick="HistoryTrack(2);" data-options="iconCls:'icon-LineRoad1'"><% =Resources.Lan.GoogleMap %></div>
		            </div>
		        </div>
	...
	<div id="vehMenuYJ" class="easyui-menu unselectable" >
		<div id="Y20" class="unselectable" data-options="iconCls:'icon-control'"><span><% =Resources.Lan.RightTerminalControl %></span>
		    <div >
		        <div class="unselectable" onclick="NavigationVehRoad(this);" data-options="iconCls:'icon-LineRoad1'"><% =Resources.Lan.GpsNavigation %></div>
		        <div class="unselectable" id="Y1602" onclick="TaskPic(this,4);" data-options="iconCls:'icon-photo'"><% =Resources.Lan.RightPhotograph %></div>
		        <div class="unselectable" id="Y65" onclick="TaskVideo(this);" data-options="iconCls:'icon-video'"><% =Resources.Lan.RealtimeVideoMonitoring %></div>
		        <div class="unselectable" id="Y204" onclick="OverSpeedSetting(this,4);" data-options="iconCls:'icon-speed'"><% =Resources.Lan.OverSpeedSetting %></div>
		        <div class="unselectable" id="Y2006" onclick="ShutdownSetting(this,4);" data-options="iconCls:'icon-acc'"><% =Resources.Lan.Shutdown %></div>
		    </div>
		</div>
		<div id="Y53" class="unselectable" data-options="iconCls:'icon-online'"><span><% =Resources.Lan.RightVehicleInformationCommand %></span>
		    <div>
		        <div id="Y5006"  class="unselectable" data-options="iconCls:'icon-LineRoad1'"><span><% =Resources.Lan.RightTrackPlayback %></span> 
		            <div>
		                <div class="unselectable" onclick="HistoryTrack(1);" data-options="iconCls:'icon-LineRoad1'"><% =Resources.Lan.BaiduMap %></div>
		                <div class="unselectable" onclick="HistoryTrack(2);" data-options="iconCls:'icon-LineRoad1'"><% =Resources.Lan.GoogleMap %></div>
		            </div>
	...
	<div id="vehMenuEnterprice" class="easyui-menu unselectable" >
		<div id="E4" class="unselectable" data-options="iconCls:'icon-control'"><% =Resources.Lan.RigthTerminalControlInstruction %>
		    <div>
		        <div id="E5001" class="unselectable" onclick="CallName(this,1);" data-options="iconCls:'icon-language'"><% =Resources.Lan.RightCallName%></div>
		        <%--<div id="E5002" data-options="iconCls:'icon-photo'"><% =Resources.Lan.RightPhotograph%></div>--%>
		    </div>
		</div>
		<div id="E2" class="unselectable" data-options="iconCls:'icon-setting'"><% =Resources.Lan.RightTerminalSettings%>
		    <div>
		        <div id="E201" class="unselectable" onclick="AccOnSetting(this,1);" data-options="iconCls:'icon-time'"><% =Resources.Lan.RightAccOnTimingInterval%></div>
		        <div id="E202" class="unselectable" onclick="AccOffSetting(this,1);" data-options="iconCls:'icon-time'"><% =Resources.Lan.RightAccOffTimingInterval%></div>
		        <div id="E204" class="unselectable" onclick="OverSpeedSetting(this,1);" data-options="iconCls:'icon-speed'"><% =Resources.Lan.OverSpeedSetting %></div>
		        <div id="E5004" class="unselectable" onclick="SchedulingCmdSetting(this,1);" data-options="iconCls:'icon-text'"><% =Resources.Lan.SendSchedulingInformation %></div>
		        <div id="E2002" class="unselectable" onclick="OilOpenOrClose(this,1,'0');" data-options="iconCls:'icon-text'"><% =Resources.Lan.OilCutOff %></div>
		        <div id="E2003" class="unselectable" onclick="OilOpenOrClose(this,1,'1');" data-options="iconCls:'icon-text'"><% =Resources.Lan.OilOpen %></div>
		    </div>
		</div>
		<div id="E53" class="unselectable" data-options="iconCls:'icon-online'"><span><% =Resources.Lan.RightVehicleInformationCommand %></span>
		    <div>
		        <div id="E5006" class="unselectable"  data-options="iconCls:'icon-LineRoad1'"><span><% =Resources.Lan.RightTrackPlayback %></span> 
		            <div>
		                <div class="unselectable" onclick="HistoryTrack(1);" data-options="iconCls:'icon-LineRoad1'"><% =Resources.Lan.BaiduMap %></div>
		                <div class="unselectable" onclick="HistoryTrack(2);" data-options="iconCls:'icon-LineRoad1'"><% =Resources.Lan.GoogleMap %></div>
		            </div>
	...
	<div id="vehMenuK1" class="easyui-menu unselectable" >
		<div class="unselectable" id="K2" data-options="iconCls:'icon-setting'"><% =Resources.Lan.RightTerminalSettings%>
		    <div>
		        <div id="K201" class="unselectable" onclick="AccOnSetting(this,5);" data-options="iconCls:'icon-time'"><% =Resources.Lan.TimingTimeComesBack %></div>
		        <div id="K5004" class="unselectable" onclick="SchedulingCmdSetting(this,5);" data-options="iconCls:'icon-text'"><% =Resources.Lan.SendSchedulingInformation %></div>
		        <div id="K406" class="unselectable" onclick="RestartSetting(this,5);" data-options="iconCls:'icon-restart'"><% =Resources.Lan.Restart %></div>
		    </div>
		</div>
		<div id="K53" class="unselectable" data-options="iconCls:'icon-online'"><span><% =Resources.Lan.RightVehicleInformationCommand %></span>
		    <div>
		        <div id="K5006" class="unselectable"  data-options="iconCls:'icon-LineRoad1'"><span><% =Resources.Lan.RightTrackPlayback %></span> 
		            <div>
		                <div class="unselectable" onclick="HistoryTrack(1);" data-options="iconCls:'icon-LineRoad1'"><% =Resources.Lan.BaiduMap %></div>
		                <div class="unselectable" onclick="HistoryTrack(2);" data-options="iconCls:'icon-LineRoad1'"><% =Resources.Lan.GoogleMap %></div>
		            </div>
	...
	<div id="divVehGrid" class="easyui-menu"  style="width:120px;">
		<div data-options="iconCls:'icon-clear'" class="unselectable" onclick="ClearVehGrid();"><% =Resources.Lan.Clear %></div>
    </div>
	<div id="divAlarmGrid" class="easyui-menu"  style="width:120px;">
		<div data-options="iconCls:'icon-clear'" class="unselectable" onclick="ClearAlarmGrid();"><% =Resources.Lan.Clear %></div>
    </div>
	<div id="divInteractiveGrid" class="easyui-menu"  style="width:120px;">
		<div data-options="iconCls:'icon-clear'" class="unselectable" onclick="ClearInteractiveGrid();"><% =Resources.Lan.Clear %></div>
    </div>
